...
func New(db *sql.DB, etcdAddrs []string, logger *log.Logger) Elector {
	if len(etcdAddrs) > 0 {
...
func newEtcdElector(addrs []string, logger *log.Logger) Elector {
	cli, err := clientv3.New(clientv3.Config{
		Endpoints:   addrs,
		DialTimeout: 5 * time.Second,
	})
	if err != nil {
		logger.Printf("failed to create etcd client, falling back to local: %v", err)
		return &localElector{logger: logger}
	}
	return &etcdElector{
		client: cli,
...
func (e *etcdElector) Acquire(ctx context.Context) error {
	...
	if err != nil {
		e.logger.Printf("etcd session creation failed, falling back to local: %v", err)
		e.client.Close()
	...
	if err := election.Campaign(ctx, "scheduler"); err != nil {
		session.Close()
		e.logger.Printf("etcd campaign failed, falling back to local: %v", err)
		e.client.Close()
		return (&localElector{logger: e.logger}).Acquire(ctx)
	}

	e.logger.Printf("leader election backend=etcd role=leader")
	metrics.DefaultRegistry.IncCounter("leader_election_total", map[string]string{
		"backend": "etcd",
		"result":  "acquired",
	})
